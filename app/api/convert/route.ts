...
const ALLOWED_INPUT: Record<string, string[]> = {
  "image-to-webp": ["image/jpeg", "image/png", "image/gif", "image/bmp", "image/tiff", "image/avif"],
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!allowed || !allowed.includes(file.type)) {
      return NextResponse.json({ error: `Unsupported format for ${tool}` }, { status: 415 });
    }
    ...
    if (tool === "image-to-webp") {
      outputBuffer = await img.webp({ quality, effort: 4, smartSubsample: true }).toBuffer();
      outputMime = "image/webp";
    } else if (tool === "webp-to-jpg") {
      outputBuffer = await img.jpeg({ quality, mozjpeg: true }).toBuffer();
      outputMime = "image/jpeg";
    } else if (tool === "webp-to-png") {
      outputBuffer = await img.png({ compressionLevel: 9 }).toBuffer();
      outputMime = "image/png";
    } else if (tool === "heic-to-jpg") {
      outputBuffer = await img.jpeg({ quality, mozjpeg: true }).toBuffer();
      outputMime = "image/jpeg";
    } else {
    ...
    const outName = encodeURIComponent(file.name.replace(/\.[^.]+$/, config.outputExt));
    ...
    return NextResponse.json({ error: "Conversion failed. Please try another file." }, { status: 500 });
  }
}
